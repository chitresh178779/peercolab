...
function SubjectWorkspace({ 
  ...
  const [expandedSubjects, setExpandedSubjects] = useState({}); // Tracks expanded state per subject
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: expandedSubjects[subject._id] ? '0.5rem' : '0' }}>
                ...
                >
                  {expandedSubjects[subject._id] ? (
                    <ChevronDown size={18} style={{ color: 'var(--accent-blue)', flexShrink: 0 }} />
                  ) : (
                    <ChevronRight size={18} style={{ color: 'var(--accent-blue)', flexShrink: 0 }} />
                  )}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  {isOwner && (
                    <button
                      onClick={() => {
                        // Expand first if collapsed when toggling the share form
                        if (!expandedSubjects[subject._id]) {
                          setExpandedSubjects(prev => ({ ...prev, [subject._id]: true }));
                        }
                        setShowShareForm(prev => ({ ...prev, [subject._id]: !prev[subject._id] }));
                      }}
                      style={{
                        background: 'none',
                        border: 'none',
